...
public class DisciplineController {
	...
	public List<User> getUserToList() {
		List<User> userToList = (this.getDiscipline() == null) ? new ArrayList<User>()
				: new ArrayList<User>(this.getDiscipline().getRegistration());
		return userToList;
	}
	...
	public boolean containsDisciplineInDepartment(String code) {
		DepartmentImpl departmentImpl = new DepartmentImpl();
		List<Department> department = departmentImpl.getDepartmentsList();

		for (Department dep : department) {
			for (Discipline discipline : dep.getDiscipline()) {
				if (code.equals(discipline.getDisciplineCode()))
					return true;
			}
		}
	...
	public void deleteDiscipline(ActionEvent acEvent) {
		...
		try {
			if (this.containsDisciplineInDepartment(this.getDiscipline()
					.getDisciplineCode())) {
				msg = new FacesMessage(
						FacesMessage.SEVERITY_ERROR,
						"Erro!",
						"Não é possível remover esta"
								+ " disciplina, pois existe um ou mais departamentos"
			...
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
					"Ocorreu um erro ao tentar remover a disciplina.");
			e.printStackTrace();
		}
	...
	public void createNewDiscipline(ActionEvent actionEvent) {
		 ...
		 * Validações
		 */
		if (this.getDiscipline().getDisciplineCode() == null
				|| this.getDiscipline().getDisciplineCode().equals("")) {
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
					"É necessário informar o código da disciplina.");
		} else if (this.getDiscipline().getDisciplineName() == null
				|| this.getDiscipline().getDisciplineName().equals("")) {
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
					"É necessário informar o nome da disciplina.");
		} else if (this.getDiscipline().getCopyLimit() == null
				|| this.getDiscipline().getCopyLimit().equals("")) {
			this.getDiscipline().setCopyLimit(0);
		} else {
			Set<User> usersTarget = new HashSet<User>(this.getUsers()
					.getTarget());
			...
			try {
				...
				msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Sucesso!",
						"Disciplina "
								+ this.getDiscipline().getDisciplineName()
								+ " criada.");
			} catch (RuntimeException e) {
				e.printStackTrace();
				msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
	...
	public void updateDiscipline(ActionEvent actionEvent) {
		...
		if (this.getDiscipline().getDisciplineCode() == null
				|| this.getDiscipline().getDisciplineCode() == "") {
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
					"É necessário informar o código da disciplina.");
		} else if (this.getDiscipline().getDisciplineName() == null
				|| this.getDiscipline().getDisciplineName() == "") {
	...
	public DualListModel<User> getDisciplinesDualList() {
		...
		if (this.getDiscipline() == null)
			return new DualListModel<User>(usersList, temp);

		Discipline disciplines = disciplineImpl.getDiscipline(this
				.getDiscipline().getDisciplineCode());
		List<User> setToListUser = new ArrayList<User>(
				disciplines.getRegistration());
		...
		for (int i = 0; i < usrArray.size(); i++) {
			if (userArray.contains(usrArray.get(i)))
				usersList.remove(i);
		}
