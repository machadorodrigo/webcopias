...
public class UserController {
	...
	private Boolean isAdmin, isBoss, isTeacher, isOperator, isSelectedAdmin,
			isSelectedBoss, isSelectedTeacher, isSelectedOperator;
	
	private Boolean isNewAdmin, isNewBoss, isNewTeacher, isNewOperator,
			isActive;
	...
	public UserController() {
		...
		if (context instanceof SecurityContext) {
			...
			if (authentication instanceof Authentication) {
				Collection<GrantedAuthority> role = authentication
						.getAuthorities();
				this.roleList = role;

				// Inicializa setters
				currentUser = ((org.springframework.security.core.userdetails.User) authentication
	...
	public Boolean getIsSelectedAdmin() {
		...
		if (user != null) {
			for (Role role : user.getRole()) {
				if (role.getName().equalsIgnoreCase("ROLE_ADM"))
					return true;
			}
		}
	...
	public Boolean getIsSelectedBoss() {
		...
		if (user != null) {
			for (Role role : user.getRole()) {
				if (role.getName().equalsIgnoreCase("ROLE_BOSS"))
					return true;
			}
		}
	...
	public Boolean getIsSelectedOperator() {
		...
		if (user != null) {
			for (Role role : user.getRole()) {
				if (role.getName().equalsIgnoreCase("ROLE_OPERATOR"))
					return true;
			}
		}
	...
	public boolean containsUserInDepartment(String registration) {
		...
		for (Department dep : department) {
			if (registration.equals(dep.getRegistration().getRegistration()))
				return true;
		}
	...
	public boolean containsUserInDiscipline(String registration) {
		...
		for (Discipline disc : discipline) {
			for (User usr : disc.getRegistration()) {
				if (registration.equals(usr.getRegistration()))
					return true;
			}
		}
	...
	public void deleteUser(ActionEvent acEvent) {
		...
		try {
			if (this.getSelectedUser().getRegistration()
	...
	public void createNewUser(ActionEvent actionEvent) {
		...
		if (this.getSelectedUser().getRegistration() == null
				|| this.getSelectedUser().getRegistration().trim() == "") {
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
					"É necessário informar a matrícula do usuário.");
		} else if (this.getSelectedUser().getName() == null
				|| this.getSelectedUser().getName().trim() == "") {
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
					"É necessário informar o nome do usuário.");
		} else if (this.getSelectedUser().getPassword() == null
				|| this.getSelectedUser().getPassword().trim() == "") {
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
			...
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
					"É necessário selecionar ao menos um grupo.");
			...
			try {
				...
				msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Sucesso!",
						"Usuário " + this.getSelectedUser().getName()
								+ " criado.");
			} catch (RuntimeException e) {
				e.printStackTrace();
				msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
